...
class MixEHR(nn.Module):
    def __init__(self, K, corpus, modality_list, stochastic_VI=True,  batch_size=1000, out='./store/'):
        ...
        # hyper-parameters
        self.alpha = torch.distributions.Gamma(torch.tensor([10.0]), torch.tensor([1.0])).sample([self.K]).T[0].to(device) # hyperparameter for prior on topic weight vectors theta
        self.beta = [torch.distributions.Gamma(torch.tensor([2.0]), torch.tensor([100])).sample([self.K,self.V[m]]).T[0].to(device)
        ...
        self.pi = torch.distributions.Gamma(torch.tensor([10.0]), torch.tensor([1.0])).sample([self.K,self.D]).T[0].to(device)
        ...
        self.w_ = torch.tensor(np.random.rand(self.K)).to(device) # survival coefficient for cox regression
    ...
    def get_prior(self, test = False):
        ...
        pi_np = pi_np*10 + 0.01
        self.pi = torch.from_numpy(pi_np).to(device)
    ...
    def SCVB0(self, batch_BOW, batch_indices, batch_C, batch_cnt_m, iter_n, m):
        ...
        for d_i, doc_id in enumerate(batch_indices):
            ...
            temp_gamma[BOW_nonzero] /= torch.exp(torch.tensor(self.H0(self.Survival_times[d_i])) * torch.exp(torch.dot(self.w_,self.exp_m[doc_id])+ (self.w_ / batch_cnt_m[d_i])))
            # normalization
        ...
        tempsurv_exp_m = self.exp_m.numpy()
        ...
        self.w_ = torch.tensor(self.cph_M.coef_).to(device)
    ...
    def inference(self, max_epoch=200, save_every=50):
        ...
        for epoch in range(0, max_epoch):
            ...
            # Initialize the survival information
            if epoch == 0:
