...
class Corpus(Dataset):
    ...
    @staticmethod
    def build_from_GDTM_fileformat(modaltiy_list, path_dict, label_path:str, column_dict, store_path=None):
        '''
        ...
        :param label_path: For each SUBJECT_ID, includes 2 features
                            Surv_time: the patient's survival time (time for the event) 
                            Censor: Censor indicator, censor = 1 event happens, censor = 0 event haven't happens
                            Headers: SUBJECT_ID,Surv_time,Censor
        ...
        def __read_labels__(patient_ids, labels):
            Survival_times = {}
            Censor_indicators = {}
            num_patients = len(patient_ids.keys())
            pbar = tqdm(labels.iterrows())
            for i, label in enumerate(pbar):
                label = label[1]
                patient_id = label['nam']
                Survival_times[patient_id] = label['Surv_time']
                Censor_indicators[patient_id] = label['Censor']
                record = patient_ids[patient_id]
                pbar.set_description("%.4f  - patient(%s)" % (100 * (i + 1) / num_patients, record))
            return Survival_times, Censor_indicators

        def __read_patients__(data_list, Survival_times, Censor_indicators,vocab_list, modaltiy_list, column_dict):
            training = {}
                ...
                with tqdm(total=num_records) as pbar:
                    for i, row in enumerate(data_m.iterrows()): # for each record, append to its Patients
                        ...
                        if patient_id not in training:
                            training[patient_id] = Corpus.Patient(patient_id, Survival_times[patient_id], Censor_indicators[patient_id], modality_num=len(modaltiy_list))
        ...
        Survival_times, Censor_indicators = __read_labels__(patient_ids, labels)
        # Process read Documents
        dataset = __read_patients__(data_list, Survival_times, Censor_indicators, vocab_list, modaltiy_list, column_dict)
    ...
    class Patient(object):
        def __init__(self, patient_id, Survival_time: int, Censor_indicator: int, words_dict:dict=None, modality_num=1):
            '''
            Create a new patient.
            '''
            self.patient_id = patient_id # index of patients, patient_id for train set and test set starts from 0
            self.Survival_time = int(Survival_time)
            self.Censor_indicator = int(Censor_indicator)
        ...
        def __str__(self): # print Patient object will return this string
            ...
            return "patient id: (%s), Words %s, Count %s, Survival time %s, Censor indicator %s" % (self.patient_id, [len(words_dict) for words_dict in self.words_dict], [Cd for Cd in self.Cd], self.Survival_time, self.Censor_indicator)
...
def run(args):
    ...
    if cmd == 'process':
        ...
        label_path = os.path.join(BASE_FOLDER, 'patient_label.csv')
        ...
        Corpus.build_from_GDTM_fileformat(modality_list, path_dict, label_path, column_dict, STORE_FOLDER)
